    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setMessage({ 
                    text: data.detail || "Failed to send password reset link. Please try again.", 
                    isError: true 
                });
            }
        } catch (error) {
    ...
    return (
        <div className="bg-black flex flex-col min-h-screen items-center justify-center text-white">
            ...
            <div className="bg-black/90 w-full max-w-md flex flex-col items-center justify-center p-6 md:p-8 rounded-[10px]">
                ...
                {message.text && (
                    <div className={`w-full mb-4 p-3 rounded-md ${message.isError ? 'bg-red-900' : 'bg-green-900'}`}>
                        {message.text}
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit} className="w-full flex flex-col gap-6">
                    ...
                    <div className="w-full flex flex-col items-center">
                        <button
                            type="submit"
                            disabled={isLoading}
                            className={`w-full bg-white text-black border-2 border-black py-2 rounded-md font-semibold hover:bg-gray-100 transition-colors ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
